	...
	<section id="pageList">
		<c:choose>
			<c:when test="${pageNum > 1}">
				<input type="button" value="이전" onclick="location.href='storeMain.st?page=${pageNum - 1}&select=${select }'">
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:choose>
				...
				<c:otherwise>
					<a href="storeMain.st?page=${i }">${i }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:choose>
			<c:when test="${pageNum < maxPage}">
				<input type="button" value="다음" onclick="location.href='storeMain.st?page=${pageNum + 1}&select=${select }'">
			</c:when>
			<c:otherwise>
